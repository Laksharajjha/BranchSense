...
fn test_engine_evaluates_clean_evidence() {
    ...
    let env = EvidenceEnvelope::new(EvidenceState::Observed, EvidenceCompleteness::new(), AnalysisProvenance::new());
    aggregator.add(BcsNormalizedEvidence::new(BcsEvidenceCategory::Collision, env, vec![], "A".into(), 45));
...
fn test_engine_abtains_on_unavailable_evidence() {
    ...
    let env = EvidenceEnvelope::new(EvidenceState::Unavailable, EvidenceCompleteness::new(), AnalysisProvenance::new());
    aggregator.add(BcsNormalizedEvidence::new(BcsEvidenceCategory::History, env, vec![], "A".into(), 50));
...
fn test_engine_caps_at_100() {
    ...
    let env = EvidenceEnvelope::new(EvidenceState::Observed, EvidenceCompleteness::new(), AnalysisProvenance::new());
    aggregator.add(BcsNormalizedEvidence::new(BcsEvidenceCategory::Collision, env.clone(), vec![], "A".into(), 60));
    aggregator.add(BcsNormalizedEvidence::new(BcsEvidenceCategory::Impact, env, vec![], "B".into(), 60));
